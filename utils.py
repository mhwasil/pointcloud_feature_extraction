...
def get_3DmFV(points, w, mu, sigma, normalize=True):
    ...
    d_pi = np.concatenate([np.max(d_pi_all, axis=1), 
                          np.sum(d_pi_all, axis=1)], axis=2)

    d_mu_all = Q_per_d * (batch_points - batch_mu) / batch_sig
    d_mu = (1 / (np.sqrt(w_per_batch_per_d))) * np.concatenate([np.max(d_mu_all, axis=1),
            np.min(d_mu_all, axis=1), 
            np.sum(d_mu_all, axis=1)], axis=2)
    d_mu = (1 / (np.sqrt(w_per_batch_per_d))) * np.concatenate([np.sum(d_mu_all, axis=1)], axis=2)
    ...
    d_sigma = (1 / (np.sqrt(2 * w_per_batch_per_d))) * np.concatenate([np.max(d_sig_all, axis=1), 
               np.min(d_sig_all, axis=1), 
               np.sum(d_sig_all, axis=1)], axis=2)
...
def get_FV(points, w, mu, sigma, normalize=True):
    """
       Compute the 3D modified fisher vectors given the gmm model parameters (w,mu,sigma) and a set of points
       For faster performance (large batches) use the tensorflow version
    ...
    # Compute derivatives
    w_per_batch_per_d = np.tile(np.expand_dims(np.expand_dims(w, 0), -1),
                                [n_batches, 1, 3])  # n_batches X n_gaussians X 3*D (D for min and D for max)
    ...
    #d_pi = np.concatenate([np.max(d_pi_all, axis=1), 
    #                       np.sum(d_pi_all, axis=1)], axis=2)
    d_pi = np.concatenate([np.sum(d_pi_all, axis=1)], axis=2)
    ...
    #            np.min(d_sig_all, axis=1), 
    #            np.sum(d_sig_all, axis=1)], axis=2)
    d_sigma = (1 / (np.sqrt(2 * w_per_batch_per_d))) * np.concatenate([np.sum(d_sig_all, axis=1)], axis=2)
